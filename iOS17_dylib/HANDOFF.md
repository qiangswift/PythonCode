...
- Telegram 12.9.3 的代理主机名默认经 `MTDNS`
  ...
  开关实际改用 `resolveHostnameNative:port:`。
...
## TelegramRotationToggle 1.0.6

- 1.0.5 实机显示头像外框被拉宽，新增区域无图标且不可点击。这证明 bar item 已插入，但 Telegram 自定义 NavigationBar 不支持 `UIBarButtonItem(customView: UIButton)` 的渲染和事件映射。
...
## TelegramRotationToggle 1.0.9

- 1.0.8 实机确认功能全部正确；遗留问题是按钮圆形背景不需要，以及返回首页后 Overlay 残留。后者源于 Telegram 未稳定转发聊天页 `viewWillDisappear:`，独立窗口不会自动跟随页面销毁。
